...
#define WINDOW_NAME		_T("Dungeons")		// ウインドウのキャプション名
...
D3D11_VIEWPORT				g_vp;					// ビューポート

int							g_nCountFPS;			// FPSカウンタ
float						g_fdeltaTime;			// Updateからの経過時間
	...
	for (;;) {
		if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE)) {
			...
			if ((dwCurrentTime - dwExecLastTime) >= (1000 / 60)) {
				g_fdeltaTime = (dwCurrentTime - dwExecLastTime) /1000;
...
HRESULT CreateBackBuffer(void)
{
	...
	// ビューポート設定
	g_vp.Width = (float)SCREEN_WIDTH;
	g_vp.Height = (float)SCREEN_HEIGHT;
	g_vp.MinDepth = 0.0f;
	g_vp.MaxDepth = 1.0f;
	g_vp.TopLeftX = 0.0f;
	g_vp.TopLeftY = 0.0f;
	g_pDeviceContext->RSSetViewports(1, &g_vp);
...
HRESULT Init(HWND hWnd, BOOL bWindow)
{
	...
	io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\meiryo.ttc", 20.0f, nullptr, io.Fonts->GetGlyphRangesJapanese());
...
{
	// 終了処理
	PostMessage(g_hWnd, WM_CLOSE, 0, 0);	// [x]が押されたように振舞う
}