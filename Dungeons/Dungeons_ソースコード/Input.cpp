...
int			CInput::m_nJoyTimer[MAX_DIRECTION];
...
//=============================================================================
// ゲームパッド スティックを正規化し少数第３桁まで返す
//=============================================================================
float CInput::GetStick(float input)
{
	input /= 32768;
	input *= 100;
...
//=============================================================================
// ゲームパッド Lスティック 一定間隔のトリガ情報取得(DIRECTION:方向, time:時間
//=============================================================================
bool CInput::GetStickLTrigger(DWORD dwjoy, STICKDIRECTION dr, int time)
{
	if (m_dwJoyCount <= 0)
		return false;
	...
	switch (dr) // 閾値以下の時falseを返す
	{
	case DR_UP:
		if (GetStick((float)GetJoyY(dwjoy)) > -0.5f)
			return false;
		break;
	case DR_DOWN:
		if (GetStick((float)GetJoyY(dwjoy)) < 0.5f)
		...
		break;
	case DR_LEFT:
		if (GetStick((float)GetJoyX(dwjoy)) > -0.5f)
			return false;
		break;
	case DR_RIGHT:
		if (GetStick((float)GetJoyX(dwjoy)) < 0.5f)
